  ...
  return BbPromise.map(
    ...
    config => {
      if (isIndividialPackaging.call(this) && _.get(config, 'cache.type') === 'filesystem') {
        ...
        return webpackCompile(clonedConfig, logStats, this.serverless.classes.Error).catch(error => {
          errors.push(error);
          return error.stats;
        });
      }
      return webpackCompile(config, logStats, this.serverless.classes.Error).catch(error => {
        errors.push(error);
        return error.stats;
      });
    },
    ...
    if (errors.length) {
      ...
      throw new this.serverless.classes.Error('Webpack compilation errors, see stats above');
    }
